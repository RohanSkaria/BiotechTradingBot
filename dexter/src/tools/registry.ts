...
import { getAlpacaMarketData, ALPACA_MARKET_DATA_DESCRIPTION } from './finance/alpaca-data.js';
import { getYahooFinanceData, YAHOO_FINANCE_DATA_DESCRIPTION } from './finance/yahoo-data.js';
import { manageWatchlistTool, MANAGE_WATCHLIST_DESCRIPTION } from './finance/manage-watchlist.js';
...
export function getToolRegistry(model: string): RegisteredTool[] {
  ...
  if (process.env.ALPACA_KEY && process.env.ALPACA_SECRET) {
    tools.push({
      ...
      tool: getAlpacaMarketData,
      description: ALPACA_MARKET_DATA_DESCRIPTION,
      compactDescription: 'Free Alpaca fallback for stock bars, latest quote, and company news.',
      concurrencySafe: true,
    });
  }
  ...
  tools.push({
    ...
    description: YAHOO_FINANCE_DATA_DESCRIPTION,
    compactDescription: 'Free Yahoo fallback for quote, fundamentals, history, recommendations, and options chains.',
    concurrencySafe: true,
  });
  ...
  if (process.env.DATABASE_URL) {
    tools.push({
      ...
      tool: manageWatchlistTool,
      description: MANAGE_WATCHLIST_DESCRIPTION,
      compactDescription: 'Manage biotech weekly_watchlist in Neon: expire_stale | list_active | add. Soft-delete only.',
      concurrencySafe: false,
    });
  }
